...
class PrivateViewsTests(TestCase):
    def setUp(self) -> None:
        ...
        payment_card = Card.objects.create(user=self.user, bank_name="Mono", type="Payment card", currency=currency)
        Card.objects.create(user=self.user, bank_name="Mono", type="Storage card", currency=currency)
    ...
    def test_retrieve_wallets(self):
        ...
        self.assertEqual(list(response.context["cards_list"]), list(cards))
        self.assertEqual(list(response.context["cash_list"]), list(cash))
    ...
    def test_retrieve_wallets_financial_turnover(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(list(response.context["accountancy_list"]), list(accountancy))
        self.assertTemplateUsed(response, "manager/monthly_accountancy.html")
